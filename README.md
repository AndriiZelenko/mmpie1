...
```commandline
$ git clone git@github.com:AndriiZelenko/mmpie1.git && cd mmpie1
```
...
The mmpie1 package is an end-to-end machine learning framework for training and deploying models. It is meant 
to help kickstart new projects, containing all the structural components necessary to train, manage and deploy models. 
Mmpie1 has been created in a modular design, so if a particular component is not needed, it can simply be deleted.
...
- **[docker](docker)**. Contains Dockerfiles for building the mmpie1 Docker image. Refer to the [Docker](./docker) 
    module for more details;
- **[mmpie1](mmpie1)**: The main package, containing all the core functionality of the project;
  - **[mmpie1/backend](mmpie1/backend)**: Contains modules for running each of the discord, gateway and 
      training servers;;
  - **[mmpie1/configs](mmpie1/configs)**: Contains the (Hydra) yaml files for model training;
  - **[mmpie1/core](mmpie1/core)**: Contains core mmpie1 packages, including:
    - **[Config](mmpie1/core/config.py)**. This class provides access to the associated 
      [config.ini](mmpie1/core/config.ini) file, used for package-level configurations;
    - **[Mmpie1Base](mmpie1/core/base.py)**. This class serves as a helper base class. It provides unified 
      logging functionality, among other features.
  - **[mmpie1/data](mmpie1/data)**. Contains modules for individual `pl.LightningDataModule` datasets;
  - **[mmpie1/models](mmpie1/models)**. Contains modules for individual `torch.nn.Module` models; also includes 
      a torch Lightning wrapper class for `torch.nn.Modules`, which can be used to wrap models during training;
  - **[mmpie1/modules](mmpie1/modules)**: Modules used within the package, including model Registry and GPT 
      classes;
  - **[mmpie1/utils](mmpie1/utils)**: Utility functions used throughout the package. Notable utilities include:
    - **[conversions](mmpie1/utils/conversions.py)**. Provides methods for converting and serializing PIL images, 
      used to pass images between servers;
    - **[logging](mmpie1/utils/logging.py)**. Provides a `default_logger` method, may be used to customize the 
      logger provided by the BaseMmpie1 class;
  - **[mmpie1/scripts](mmpie1/scripts)**: Standalone scripts built on top of the mmpie1 package; the project 
      starts with a training script that demonstrates how to set up a training pipeline to store models to the model 
      registry;
- **[tests](tests)**: Contains unit tests for the mmpie1 package. Refer to the [Unit Tests](#Unit-Tests) section for 
    more details;
...
`docs/_build/simplepdf/Mmpie1.pdf` respectively.
...
from [GithubActions](https://github.com/AndriiZelenko/mmpie1/actions). 

# End-to-End Demo

The end-to-end application demo allows training and deploying models through a private discord server. It is meant to 
showcase the full functionality of the mmpie1 package, and provide a starting point for deploying your own models.
...
Finally, add your discord bot token to your [config.ini](mmpie1/core/config.ini) file under API_KEYS/DISCORD. You may 
...
[config.ini](mmpie1/core/config.ini) file under API_KEYS/OPENAI.
...
[config.ini](mmpie1/core/config.ini) file to match, or pass them in as command line arguments to each other server. 
  - The default configs assume that output data (MLFlow registry, hydra training runs, tensorboard logs, standard 
debug logs, etc.) will be stored in `${HOME}/mmpie1` and, for unit tests, that the project has been installed in 
`${HOME}/projects/mmpie1`. If you wish to change these locations, you will need to update the same 
[config.ini](mmpie1/core/config.ini) file accordingly. To create the default directories automatically, you may run 
...
in a completely separate environment, and configure each training job to get a separate GPU. 
  - If, at any point, you get an error saying `mmpie1` cannot be found, remember to add the mmpie1 path to your 
