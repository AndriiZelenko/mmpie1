...
from mmpie1.backend.training.connection_client import ConnectionClient as TrainingConnection
from mmpie1.backend.training.types import TrainingRunInput
from mmpie1.utils import default_logger


class TrainingServer(Mmpie1Base):
    """Mmpie1 Training Server

    The Mmpie1 Training Server provides a unified interface for starting training runs. It is used by the gateway
    ...
    code::
        ...
        $ python -m gunicorn -w 1 -b localhost:8081 -k uvicorn.workers.UvicornWorker \
            mmpie1.backend.training.training_server:app

    """

    logger = default_logger(
        name="mmpie1.backend.training.training_server.TrainingServer",
