...
from mmpie1 import Mmpie1Base
from mmpie1.backend.deployment import DeploymentServer
from mmpie1.backend.gateway.connection_client import ConnectionClient as GatewayConnection
from mmpie1.backend.gateway.types import (
...
from mmpie1.backend.training import TrainingServer
from mmpie1.data import MNIST
from mmpie1.modules import GPT, Registry
from mmpie1.types import Message
from mmpie1.utils import ascii_to_pil, default_logger, ifnone, pil_to_ascii


class GatewayServer(Mmpie1Base):
    """Mmpie1 Gateway Server

    The Mmpie1 Gateway Server provides a unified interface for communicating with the Mmpie1 backend. It is used
    ...
    code::
        ...
        $ python -m gunicorn -w 1 -b localhost:8081 -k uvicorn.workers.UvicornWorker \
            mmpie1.backend.gateway.server:app
    ...
    def app(self):
        ...
        def chat(payload: ChatInput):
            ...
            if self.gpt is not None:
                ...
                response = Message(sender="mmpie1", text="Sorry, I don't know how to chat yet.")
        ...
        def debug(payload: DebugInput):
            ...
            instructions = (
                ...
                f"mmpie1 project. You will talk with the mmpie1 application developers and answer their "
