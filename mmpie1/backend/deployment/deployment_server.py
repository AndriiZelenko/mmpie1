...
from mmpie1.backend.deployment.connection_client import ConnectionClient as DeploymentConnection
from mmpie1.backend.deployment.types import ClassifyIDInput, ClassifyImageInput, LoadModelInput
from mmpie1.data import MNIST
from mmpie1.modules import Registry
from mmpie1.utils import ascii_to_pil, default_logger, pil_to_ascii, pil_to_ndarray, tensor_to_pil


class DeploymentServer(Mmpie1Base):
    """Mmpie1 Deployment Server

    The Mmpie1 Deployment Server provides a unified interface for deploying models. It is used by the gateway server
    to deploy models.
    ...
    code::
        ...
        $ python -m gunicorn -w 1 -b localhost:8083 -k uvicorn.workers.UvicornWorker \
            mmpie1.backend.deployment.deployment_server:app
